...
export const fileSystemSlice = createSlice({
	...
	reducers: {
		addFolderNode: (state, action: PayloadAction<AddFolderParams>) => {
			...
			if (parentNode) {
				const nameExists = parentNode.child?.some(
		...
		addFileNode: (state, action: PayloadAction<AddFileParams>) => {
			const parentId = action.payload.parentId;
			const name = action.payload.name;
			const lang_code = action.payload.lang_code;
			...
			if (parentNode) {
				...
				if (nameExists) {
					alert(
						`A file with the name "${name}" already exists in this directory.`
					);
					return;
				}
...
export const { addFolderNode, addFileNode, deleteNode } =
